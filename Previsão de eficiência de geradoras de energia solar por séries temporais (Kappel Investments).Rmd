---
title: "Kappel Investments - Análise da eficiência energética de geradoras solares compartilhadas do estado de São Paulo por séries temporais (NAVI Tech Journey)"
output:
  html_document:
    df_print: paged
---
...
FATOR_CAPACIDADE_2022 <- read_delim("/Users/haddad/Downloads/FATOR_CAPACIDADE_2022.csv", delim = ";", escape_double = FALSE, trim_ws = TRUE)
...
#Criação de um dataset com os fatores condensados por dia (o fator do dia é a soma da energia produzida dividida pela soma das capacidades diárias das 4 usinas)
...
forecast::autoplot(previsao, series = "Conjunto de Teste") + ylab("Fator de capacidade das geradoras solares") + xlab("Tempo")

#Cálculo do valor médio e do desvio padrão do conjunto de dados (utilizado para mensurar a área necessária para aluguel por parte do cliente em função do seu consumo médio)
mean(time_series$fator_capacidade)
```
```{r}
#Com a análise da série temporal, pode-se realizar previsões de através de modelos estatísticos, as quais auxiliam o cliente a mensurar sua expectativa de geração de energia renovável para o mês seguinte. Caso a produção tenha uma expectativa de queda, espera-se um comportamente mais econômico por parte do cliente, o qual evitaria a necessidade de consumo de energia do mercado regulado/distribuidoras padrões.
```
